...
func (b *Board) genHash() {
	...
	for whiteBB > 0 {
		sq := whiteBB.PopLSB()
		idx := pieceSquareIdx(sq, b.pieces[sq], White)
		newHash ^= zVals.pieceSquares[idx]
	}
	for blackBB > 0 {
		sq := blackBB.PopLSB()
		idx := pieceSquareIdx(sq, b.pieces[sq], Black)
		newHash ^= zVals.pieceSquares[idx]
	}

	newHash ^= zVals.castleRights[b.castleRights]

	if b.enPassantPossible() {
		newHash ^= zVals.enPassantFiles[b.enPassantSq%8]
	}

	if b.whoseTurn == White {
		newHash ^= zVals.whiteToMove
	}
	b.hash = newHash
}

// BROKEN (I THINK CASTLE RIGHTS IDK THO)
func (b *Board) updateHash(m Move, moving, captured Piece, oldCastleRights uint8, hashedEPSq SquareOrNone) {
	newHash := b.hash
	...
	if captured > 0 {
		...
		newHash ^= zVals.pieceSquares[pieceSquareIdx(m.GetTo(), captured, capturedColor)]
	}
	// Move piece to new square
	newHash ^= zVals.pieceSquares[pieceSquareIdx(m.GetFrom(), moving, movingColor)]
	newHash ^= zVals.pieceSquares[pieceSquareIdx(m.GetTo(), moving, movingColor)]
	...
	if m.HasFlag(Castle) && !m.HasFlag(Promotion) {
		...
		newHash ^= zVals.pieceSquares[pieceSquareIdx(rookFrom, Rook, movingColor)]
		newHash ^= zVals.pieceSquares[pieceSquareIdx(rookTo, Rook, movingColor)]
	}

	// If previous en passant square was saved in the hash
	if hashedEPSq > NoSq {
		// Remove en passant square
		file := uint8(hashedEPSq % 8)
		newHash ^= zVals.enPassantFiles[file]
		if m.HasFlag(EnPassant) && !m.HasFlag(Promotion) {
			// Remove the passant-ed pawn
			rank := m.GetTo() / 8
			...
			sq := ConvertRankFile(rank, file)
			newHash ^= zVals.pieceSquares[pieceSquareIdx(sq, Pawn, capturedColor)]
		}
	} else if b.enPassantPossible() {
...
func (b *Board) enPassantPossible() bool {
	if b.enPassantSq == NoSq {
		return false
	}
	whiteBB, blackBB := b.ColorBitboards()

	epFile := uint8(b.enPassantSq % 8)
	rank := uint8(b.enPassantSq/8 - 1)
	movingColorBB := whiteBB
	if b.whoseTurn == Black {
		rank += 2
		movingColorBB = blackBB
	}

	if epFile > A {
		idx := ConvertRankFile(rank, epFile-1)
		if b.pieces[idx] == Pawn && movingColorBB.QuerySquare(idx) {
			return true
		}
	}
...
func pieceSquareIdx(sq Square, piece Piece, color int) int {
	piece_kind := int((piece - 1) * 2)
	if color == White {
		piece_kind++
	}
	return 64*piece_kind + int(sq)
}

// OLD WAY OF GENERATING HASHING NUMBERS (BEFORE HARDCODING THEM)
/*
func ZobristInit() {
	...
	zVals.whiteToMove = util.RandU64()
}
*/

// HARDCODED RANDOM NUMBERS FOR HASH GENERATION
// Polyglot books require certain numbers for zobrist hashing
// (the rest of this file is very long)

// Using this struct as a kind of "namespace"
var zVals = struct {
	whiteToMove    uint64      // If it's white's turn
	enPassantFiles [8]uint64   // Each possible file for en passant
	castleRights   [16]uint64  // Each combination of castle rights
	pieceSquares   [768]uint64 // Every piece on every square
}{
